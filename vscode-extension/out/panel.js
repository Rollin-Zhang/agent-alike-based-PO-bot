...
class TicketPanel {
    ...
    async refresh() {
        try {
            ...
            // 依時間排序（最新的在前）
            this.tickets.sort((a, b) => new Date(b.metadata.updated_at).getTime() - new Date(a.metadata.updated_at).getTime());
     ...
     * 核准票據
     */
    async approveTicket(ticketItem) {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`票據 ${ticket.id.substring(0, 8)} 已核准${config.send.dryRun ? ' (乾跑模式)' : ''}`);
            // 刷新列表
            await this.refresh();
        }
        catch (error) {
    ...
    async viewTicket(ticketItem) {
        ...
        try {
            ...
            // 創建並顯示詳情面板
            const panel = vscode.window.createWebviewPanel('ticketDetail', `票據詳情 - ${ticket.id.substring(0, 8)}`, vscode.ViewColumn.One, {
                enableScripts: true
            });
     ...
     * 產生票據詳情 HTML
     */
    generateTicketDetailHTML(ticket) {
