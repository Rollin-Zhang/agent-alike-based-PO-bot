...
import { ModelResponse, ProcessingError } from './types';
...
export class ChatInvoker {
    ...
    constructor(logger: Logger) {
        this.logger = logger;
        this.preferredModel = Config.get().model.preferred;
     ...
     * 調用 VS Code 內建聊天模型
     */
    async invokeChatModel(prompt: string, options?: {
        maxTokens?: number;
        temperature?: number;
        timeout?: number;
     ...
     */
    private async callVSCodeChatAPI(prompt: string, options?: {
        maxTokens?: number;
        temperature?: number;
        timeout?: number;
    }): Promise<ModelResponse> {
     ...
     */
    private normalizeModelMeta(model: any): { provider: string; modelId: string; display: string } {
        const provider = 'vscode.lm';
        const candidates: string[] = [];
        ['id','modelId','family','name'].forEach(k => { if (model && model[k]) candidates.push(String(model[k])); });
     ...
     * 分類模型錯誤
     */
    private classifyModelError(error: any): ProcessingError {
     ...
     */
    private async selectModelStrict(): Promise<vscode.LanguageModelChat> {
