...
async function deriveReplyTicketFromTool(
  ...
  // --- GATE 4: tool_verdict must be PROCEED ---
  // Rule: outputs takes precedence over final_outputs
  let tool_verdict;
  if (outputs !== null && outputs !== undefined) {
    // outputs exists, use it (even if malformed)
    tool_verdict = outputs.tool_verdict;
  } else {
    // fallback to final_outputs
    tool_verdict = toolTicket.final_outputs?.tool_verdict;
  }

  if (tool_verdict !== 'PROCEED') {
    return { 
      created: false, 
      reason: tool_verdict === undefined ? 'missing_tool_verdict' : 'gate_tool_verdict_not_proceed'
    };
  }

  // --- IDEMPOTENCY: Check if already derived ---
  const existingDerived = readDerived(toolTicket);
  if (existingDerived?.reply_ticket_id) {
    return { 
      created: false, 
      recovered: false,
  ...
  if (orphanReplies && orphanReplies.length > 0) {
    ...
    writeDerived(toolTicket, { reply_ticket_id: orphanId });
  ...
  // Use outputs for template, fallback to final_outputs if outputs is null/undefined
  const templateOutputs = outputs !== null && outputs !== undefined 
    ? outputs 
    : toolTicket.final_outputs || {};
  ...
  writeDerived(toolTicket, { reply_ticket_id: replyTicketId });
